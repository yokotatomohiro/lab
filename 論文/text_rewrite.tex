...
\begin{figure}[htbp]
  ...
  \end{center}
\end{figure}
From this result, the outline of $\widetilde{F}(m), z_{1}$ and $z_{2}$ when optimizing (\ref{after_wolf}) did not change from Fig.\ref{fig:minimum1}, so we seem that removing the variable $t$ does not affect the result.

\section{Conclsion}
本稿では, $\ell{l}_{1}$-normのQUBO形式での定式化を提案し, 定式化を見直すことで変数の数を減らせること確認した.
