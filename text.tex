...
\abst{
  本稿では量子アニーリングを含むイジングモデルを用いたアニーリング法でのスパース推定を可能にするために$\ell_{1}$ノルムのQUBO形式での定式化を提案する。定式化には、近年ReLUタイプ関数のQUBO形式を導出するために用いられたLegendre変換とWolfeの双対定理を利用した。さらに$\ell_{1}$ノルムの場合、これらの変換を素朴に適用した場合、冗長変数が得られることが明らかになった。そして冗長変数を取り除くことでより簡略化されたQUBO形式を導出することができた。
}
